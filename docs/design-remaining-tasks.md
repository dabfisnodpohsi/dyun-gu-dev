...
| CFG-02 | 未开始 | `dg-media` 四个 element 参数 validator | `media_decode/encode/resize/osd` 拒绝未知字段、错误类型、零尺寸和非法 box/color；创建与校验复用同一解析逻辑 | 无 |
| CFG-03 | 未开始 | `dg-stream` 四个 element 参数 validator | `rtsp_src/httpflv_src/rtmp_sink/webrtc_sink` 在加载期校验 URL、协议、队列、背压、track 和未知字段，不打开网络连接 | 无 |
...
| SYS-01 | 未开始 | `dg-openvino-sys` 分层 | FFI/link 只在 `-sys`；`dg-openvino` 保持 safe wrapper；默认构建无 SDK | 无 |
...
| ID | 状态 | 独立 PR 范围 | 验收条件 | 依赖 |
|---|---|---|---|---|
| APP-01 | 未开始 | CLI/C API 链接 media/stream registry | feature-gated 链接 `dg-media`/`dg-stream`；`list-elements` 和配置加载能发现八个 element；默认 build 仍无外部 SDK | CFG-02、CFG-03 |
