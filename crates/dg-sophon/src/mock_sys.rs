...
/// Imports an already allocated device address without taking ownership.
pub unsafe fn bm_mem_from_device(device_addr: u64, size: u64) -> bm_device_mem_t {
    bm_device_mem_t {
        ptr: usize::try_from(device_addr)
            .ok()
            .map_or(ptr::null_mut(), |value| value as *mut u8),
        size: usize::try_from(size).unwrap_or_default(),
    }
}
